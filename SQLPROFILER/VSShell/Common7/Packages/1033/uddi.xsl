...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/TR/WD-xsl">
	<xsl:template match="/">
		...
		<p _locID="L_string02_Text">The UDDI servers available on your local area network are listed below. Click the server link to browse that server.</p>
        <table class="listpage" cellpadding="3" cellspacing="1" frame="void" bordercolor="#ffffff" rules="rows" width="100%" align="center">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<tr>
						<td class="tbltext" colspan="2" _locID="L_string06_Text">No UDDI servers were found on the local network.</td>
					</tr>
				</xsl:otherwise>
			</xsl:choose>
		</table>
